...
import {
  ...
  TerminalSquare, Sun, Moon, ListTodo,
} from 'lucide-react';
...
export function AppSidebar() {
  ...
  return (
    <div
      className={`${open ? 'w-60' : 'w-14'} shrink-0 flex flex-col bg-card border-r border-border transition-[width] duration-200 ease-in-out overflow-hidden`}
    >
      {/* ── logo + toggle ── */}
      <div className={`h-14 flex items-center border-b border-border shrink-0 ${open ? 'px-3 gap-2' : 'justify-center'}`}>
        {open ? (
          <>
            ...
            <span className="font-bold tracking-tight flex-1 truncate text-sm">DOP Cockpit</span>
            <button
      ...
      {/* ── nav ── */}
      <nav className={`flex-1 overflow-y-auto py-3 space-y-0.5 ${open ? 'px-2' : 'flex flex-col items-center px-0 gap-0.5 py-3'}`}>
        <NavItem icon={LayoutGrid} label="Workspaces"     to="/"               open={open} active={at('/', true)} />
        <NavItem icon={Plus}       label="Nova Workspace" to="/workspaces/new"  open={open} active={at('/workspaces/new')} />

        {/* workspace context */}
        {wsId && workspace && (
          <>
            <div className={`${open ? 'mx-1' : 'w-8'} my-2 border-t border-border/50`} />
            {open && (
              <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60 px-3 mb-1 truncate">
                {workspace.name}
              </p>
            )}
            <NavItem
              icon={ListTodo}
              label="Demandas"
              to={`/workspaces/${wsId}/demands`}
              open={open}
              active={at(`/workspaces/${wsId}/demands`)}
            />
            <NavItem
              icon={Settings}
              label="Configurar"
              to={`/workspaces/${wsId}`}
              open={open}
              active={location.pathname === `/workspaces/${wsId}` || at(`/workspaces/${wsId}/edit`)}
            />
          </>
        )}
      </nav>
      ...
      {/* ── bottom: theme toggle ── */}
      <div className={`border-t border-border py-3 shrink-0 ${open ? 'px-2' : 'flex flex-col items-center px-0'}`}>
        <ActionItem
